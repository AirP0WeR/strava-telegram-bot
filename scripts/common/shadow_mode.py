...
class ShadowMode(object):
    ...
    @run_async
    def send_message(self, bot, message, parse_mode='Markdown', disable_web_page_preview=True,
                     disable_notification=False, reply_markup=None):
        try:
            if self.bot_variables.shadow_mode:
                bot.send_message(chat_id=self.bot_variables.shadow_mode_chat_id, text=message,
